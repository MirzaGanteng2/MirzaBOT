...
Badmin: 'Bot Harus Jadi Admin Kalo Mau Menggunakan Fiturnya', 
        ...
        await Ryuu.setStatus(`KadekBotz V3 | ⏰${uptimer}`).catch((_) => _);
        settingstatus = new Date() * 1;
        }}
        const troli =  {key: { fromMe: false,remoteJid: "status@broadcast", participant: '0@s.whatsapp.net'}, message: {orderMessage: {itemCount: 2022, status: 200, thumbnail: fakeRyuu, surface: 200, message: `Made By ${ownername}`, orderTitle: 'Kadeka', sellerJid: '0@s.whatsapp.net'} } }        
//═════════[ VERIFY ]══════════//
...
//═════════[ Button ]══════════//
const katalog = (teks) => {
             res = Ryuu.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© Kadek Botz_*", "thumbnail": thumb, "surface": 'CATALOG' }}, {quoted:troli})
        ...
        const fakegroup = (teks) => {
            Ryuu.sendMessage(from, teks, text, {
                quoted: {
                    key: {
                        fromMe: false,
                        participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6285858939020-1604595598@g.us" } : {})
                    },
                    message: {
                        "imageMessage": {
                            "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc",
...
𝙋𝙚𝙣𝙘𝙚𝙩 𝘽𝙪𝙩𝙩𝙤𝙣 𝘿𝙞 𝘽𝙖𝙬𝙖𝙝 𝙄𝙣𝙞 𝙐𝙣𝙩𝙪𝙠 𝙈𝙚𝙢𝙞𝙡𝙞𝙝 𝙈𝙚𝙣𝙪`, 
            ...
            },
            {
              "title": "SOURCE CODE [💻]",
              "rowId": `${prefix}sc`, 
              "description": `Menampilkan Script Bot KadekBotz`
            },
            {
              "title": "BIG THANKS TO [🏆]",
              "rowId": `${prefix}tqto`, 
...
╰─⬣「 𝙍𝙮𝙪𝘽𝙤𝙩𝙯 3.0.0 」⬣`
teks =
`KadekBotz 3.0.0 | Create By Kadek Botz
${tampilTanggal}`
...
╭─⬣
│✾ Owner : https://wa.me/6285858939020
╰─⬣`
teks =
`KadekBotz 3.0.0 | Create By Kadek Botz
${tampilTanggal}`
...
case 'blackpink':
                    ...
                    case 'horrorblood':
                    if (!isUser) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: troli})
                    if (args.length == 0) return reply(`Example: ${prefix + command} Kadek Botz`)
                    ini_txt = args.join(" ")
...
Ryuu.sendMessage(from, goo, image, {quoted: troli, caption: 'Nih Jangan Lupa Subscribe YT : Kadek Botz'})
break
case 'cecanmalaysia':
if (!isUser) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: troli})
...
reply(mess.wait)
goo = await getBuffer(huft.result.url)
Ryuu.sendMessage(from, goo, image, {quoted: troli, caption: 'Nih Jangan Lupa Subscribe YT : Kakek Botz'})
break
case 'cecanchina':
if (!isUser) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: troli})
...
Ryuu.sendMessage(from, buffer, video, {quoted: troli, caption: 'Nih Jangan Lupa Subscribe YT : Kadek Botz'})
break
case 'asupanukhti':
if (!isUser) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: troli})
...
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
          ...
          pll = `Nih Jangan Lupa Subscribe YT : Kadek Botz`
...
//══════════[ Fitur Sound  ]══════════//
      ...
      teks =`KadekBotz 3.0.0 | Create By Kadek Botz`
...
+ 'VERSION:3.0\n'
+ `FN:Kadek\n`
+ `ORG: Creator KadekBotz ;\n`
+ `TEL;type=CELL;type=VOICE;waid=6285858939020:6285858939020\n`
+ 'END:VCARD'.trim()
